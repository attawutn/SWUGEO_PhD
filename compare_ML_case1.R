...
writeRaster(rf_prediction,'test_rf_untune.tif',options=c('TFW=YES'))

svm_prediction = predict(ras, model=model_svmRadial)
writeRaster(svm_prediction,'test_svm_untune.tif',options=c('TFW=YES'))

xgb_prediction = predict(ras, model=xgb_model)
writeRaster(xgb_prediction,'test_xgb_untune.tif',options=c('TFW=YES'))